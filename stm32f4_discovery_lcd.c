...
/**
  ...
  * @param Ypos: specifies the Y position.
  * @param Length: line length.
  * @param Angle: line Angle[0-360]
  * @retval None
  */
void LCD_DrawLineByAngle(uint16_t Xpos, uint16_t Ypos, uint16_t Length, int Angle)
{
  uint32_t i = 0;
...
//Davide: needed to fix 1044;
       if(X < Length || Y < Length){
          X = X + 1;
          Y = Y + 1;
       }
       dx = floor( (X * sin(Angle*PI/180)) );
       dy = floor( (Y * cos(Angle*PI/180)) );
